 ...
 *
 */
function tidyUpString(strArr) { return strArr.map(str => str.trim().replace(///g, '').toLowerCase());}
...
function validate(num) { return typeof num === 'number' && num % 2 === 0 && num <= 100;
}
...
function remove(arr, index) { return arr.filter((_, idx) => idx !== index);
}
...
*/
function formatPercentage(arr) {return arr.map(num => {
    const percentage = num > 100 ? 100 : num;
